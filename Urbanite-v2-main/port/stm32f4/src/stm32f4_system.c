...
void port_system_delay_ms(uint32_t ms) 
{
  ...
  while ((port_system_get_millis() - tickstart) < ms) //No hace nada durante
...
bool stm32f4_system_gpio_read(GPIO_TypeDef * p_port, uint8_t pin){
      bool value;
      return value=(bool)(p_port->IDR & BIT_POS_TO_MASK(pin));
...
void stm32f4_system_gpio_toggle(GPIO_TypeDef *p_port, uint8_t pin) {
  ...
  if (stm32f4_system_gpio_read(p_port, pin)==true){
